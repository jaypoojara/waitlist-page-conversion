...
function replacePlaceholders(template: string, link: string): string {
  return template
    .replace(/{product}/g, WAITLIST_CONFIG.productName)
    .replace(/{link}/g, link);
}
...
export function ShareButtons({ referralLink }: ShareButtonsProps) {
  const { social, productName } = WAITLIST_CONFIG;

  const twitterText = replacePlaceholders(social.twitter, referralLink);
  const linkedinUrl = referralLink;
  const whatsappText = replacePlaceholders(social.whatsapp, referralLink);
  const emailSubject = social.email.subject.replace(/{product}/g, productName);
  const emailBody = replacePlaceholders(social.email.body, referralLink);
